...
export default function HomePage() {
  ...
  return (
    <main className="overflow-x-hidden">
      ...
      {/* 10X PERSONALIZATION — Event Date Capture */}
      <EventDateCapture onDateSelected={setEventDate} />
      ...
      <section className="pt-16 pb-8 bg-white">
        ...
        <RentalRow title={t('Water Slides', 'Toboganes de Agua')} emoji="💦" rentals={waterSlides} eventDate={eventDate} />
        <RentalRow title={t('Combo Units', 'Unidades Combo')} emoji="🎉" rentals={comboUnits} eventDate={eventDate} />
        {packages.length > 0 && <RentalRow title={t('Party Packages', 'Paquetes de Fiesta')} emoji="🎪" rentals={packages} eventDate={eventDate} />}
        {tents.length > 0 && <RentalRow title={t('Tents', 'Carpas')} emoji="⛺" rentals={tents} eventDate={eventDate} />}
        {tablesChairs.length > 0 && <RentalRow title={t('Tables & Chairs', 'Mesas y Sillas')} emoji="🪑" rentals={tablesChairs} eventDate={eventDate} />}
      ...
      {/* CTA — personalized when event date is set */}
      <section className="relative py-28 px-4 bg-[#0d2340] overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          {eventDate ? (
            <>
              <span className="inline-block bg-green-500/20 text-green-400 text-xs font-black uppercase tracking-widest px-4 py-2 rounded-full mb-6">
                🎉 {eventDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
              </span>
              <h2 className="text-5xl sm:text-7xl font-black text-white mb-6 leading-tight">
                {t("LET'S MAKE", 'HAGAMOS DE')}<br />
                <span className="text-[#f5a623]">{t('IT EPIC!', '¡ESO ÉPICO!')}</span>
              </h2>
              <p className="text-white/60 text-xl mb-10">
                {t("Your date looks great — lock it in with just a 25% deposit.", 'Tu fecha se ve perfecta — asegúrala con solo el 25% de depósito.')}
              </p>
            </>
          ) : (
            <>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/rentals" className="bg-[#f5a623] hover:bg-[#e09610] text-white font-black px-12 py-5 rounded-2xl text-xl transition-all shadow-2xl hover:scale-105">
              📅 {eventDate
                ? t(`Book for ${eventDate.toLocaleDateString('en-US',{month:'short',day:'numeric'})}`, `Reservar para ${eventDate.toLocaleDateString('en-US',{month:'short',day:'numeric'})}`)
